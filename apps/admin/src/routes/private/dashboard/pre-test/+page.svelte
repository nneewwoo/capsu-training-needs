<script lang="ts">
  ...
  $effect(() => {
    ...
    const PRETEST_CHANNEL = data.supabase
      .channel('pretest-changes')
      .on(
        'postgres_changes',
        {
          event: '*',
          schema: 'base',
          table: 'pretest_responses'
        },
        (payload) => {
    ...
    return () => {
      data.supabase.removeChannel(PRETEST_CHANNEL)
    }
  })
</script>
